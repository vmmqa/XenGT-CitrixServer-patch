...
public class EventCallback implements INotificationCallback {
    ...
    public EventCallback(IDistributionClient client, ASDCConfiguration config) {
        this.client = client;
        this.config = config;
    }
   ...
   /*     List<IArtifactInfo> artifacts = new ArtifactInfoExtractor().extract(data);
        ...
        boolean success =
                getArtifactDownloadManager().downloadArtifacts(data, artifacts);

        if (success) {
            success = getArtifactDeploymentManager().deploy(data, artifacts);
        }*/
        boolean success = true;

        String statusString = success ? "SUCCESS" : "FAILURE";
        System.out.print("Processed distribution " + data.getDistributionID() + "  (" + statusString + ")");
    }
